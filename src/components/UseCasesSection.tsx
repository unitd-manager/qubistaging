...
const UseCasesSection = () => {
  ...
  const eyebrow = data?.section?.attributes?.template ?? "Use cases";
  const heading = data?.section?.attributes?.section_title ?? "Automation that fits your business";

  const useCases = (data?.items ?? []).map((item) => ({
