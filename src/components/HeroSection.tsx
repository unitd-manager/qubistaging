...
const HeroSection = () => {
  ...
  const heroImage = liveHeroImage ?? bootstrappedHero?.image;
  const badge = a?.template ?? bootstrappedHero?.badge ?? "Agentic Automation Platform";
  const heading = a?.section_title ?? bootstrappedHero?.heading ?? "Design and orchestrate enterprise workflows with qubi";
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="max-w-2xl">
            ...
            {heading.includes("qubi") ? (
              heading.split("qubi").map((part, index) => (
                <React.Fragment key={index}>
                  {part}
                  {index < heading.split("qubi").length - 1 && (
