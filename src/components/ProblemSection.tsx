...
const ProblemSection = () => {
  ...
  const heading = data?.section?.attributes?.section_title ?? "Enterprise work is too fragmented to automate with bots alone";
  const subheading = stripHtml(data?.section?.attributes?.description) || "Most enterprises struggle with siloed tools, manual processes, and limited scalability — making true end-to-end automation impossible.";

  const problems = (data?.items ?? []).map((item) => ({
