...
const OutcomesSection = () => {
  ...
  const eyebrow = data?.section?.attributes?.template ?? "Platform outcomes";
  const heading = data?.section?.attributes?.section_title ?? "Built for measurable operational impact";

  const outcomes = (data?.items ?? []).map((item) => ({
