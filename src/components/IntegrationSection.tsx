...
const IntegrationSection = () => {
  ...
  const eyebrow = data?.section?.attributes?.template ?? "Integrations";
  const heading = data?.section?.attributes?.section_title ?? "Works across your existing stack";
  const subheading = stripHtml(data?.section?.attributes?.description) || "Connect to the systems you already use — from enterprise ERPs to modern AI APIs — with pre-built connectors and extensible integration layers.";

  const integrations = (data?.items ?? []).map((item) => ({
