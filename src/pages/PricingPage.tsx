  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative container mx-auto px-4 lg:px-8 py-12 lg:py-16 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight leading-[1.1] text-foreground max-w-4xl mx-auto">
            {heroHeading.includes("work completed") ? (
              heroHeading.split("work completed").map((part, index) => (
                <React.Fragment key={index}>
                  {part}
                  {index < heroHeading.split("work completed").length - 1 && (
            ...
            <div className="mt-12 p-8 rounded-2xl bg-primary/5 border border-primary/20 text-center">
              <p className="text-lg font-semibold text-foreground">
                {execCalloutH.includes("We sell execution") ? (
                  execCalloutH.split("We sell execution.").map((part, index) => (
                    <React.Fragment key={index}>
                      {part}
                      {index < execCalloutH.split("We sell execution.").length - 1 && (
      ...
      <section className="py-12 lg:py-16 bg-surface-elevated border-y border-border">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl mx-auto text-center mb-16">
            <span className="text-primary text-base font-semibold uppercase tracking-widest">{cmpEyebrow}</span>
            <h2 className="mt-4 text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-foreground">
              {cmpHeading}
            </h2>
          </div>
            ...
            <table className="w-full text-base">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-5 pr-8 font-semibold text-foreground text-lg">Aspect</th>
                  <th className="text-center py-5 px-4 font-semibold text-muted-foreground text-lg">Workflow Automation Tools</th>
                  <th className="text-center py-5 px-4 font-semibold text-primary text-lg">qubi</th>
                </tr>
              </thead>
              <tbody>
                {compRows.map((row) => (
                  <tr key={row.aspect} className="border-b border-border/50 hover:bg-white transition-colors">
                    <td className="py-5 pr-8 font-medium text-foreground">{row.aspect}</td>
                    <td className="py-5 px-4 text-center text-muted-foreground">{row.them}</td>
                    <td className="py-5 px-4 text-center text-primary font-medium">{row.us}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="relative container mx-auto px-4 lg:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-foreground max-w-2xl mx-auto">
            {ctaHeading.includes("your workflow") ? (
              ctaHeading.split("your workflow").map((part, index) => (
                <React.Fragment key={index}>
                  {part}
                  {index < ctaHeading.split("your workflow").length - 1 && (
