...
class StrapiAPIService {
  ...
  private getMediaUrl(media: unknown): string | undefined {
    ...
    if (
      ...
      return getStrapiMediaUrl((media as { attributes: Record<string, unknown> }).attributes.url as string);
    }
    if (
      this.isObject(media) &&
      'data' in media &&
      ...
      return getStrapiMediaUrl(((media as { data: Record<string, unknown> }).data as { attributes: Record<string, unknown> }).attributes.url as string);
    }
    return undefined;
  }
    ...
    try {
      ...
      for (const s of slugsToTry) {
        ...
        for (const q of queries) {
          const response = await api.get<unknown>(q);
  ...
  async fetchRedirects(): Promise<RedirectRule[]> {
    try {
      const response = await api.get<{ data?: Array<{ id?: number | string; attributes?: Record<string, unknown> } | Record<string, unknown>> }>('/redirects', {
        params: {
          filters: {
            isActive: {
              $eq: true,
